...
from utils.helper2 import Helper
import random
import logging
...
class ImageHelper(Helper):
    ...
    def get_mix_batch(self, bptt, adversarial_index=-1, evaluation=False):
        # 测试的时候选取同时包含后门和干净数据的数据集batch测试
        images, targets = bptt
        ...
        for index in range(0, len(images)):
            if evaluation:  # 投毒一部分数据
                if index < self.params['test_batch_size']*self.params['poison_frac']:
        ...
        return new_images, new_targets, poison_count, data_count
