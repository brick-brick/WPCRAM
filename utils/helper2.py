...
class Helper:
    ...
    def geometric_median_update(self, target_model, submit_params_update_dict,agent_name_keys, num_samples_dict, foolsgold_weight, maxiter=4, eps=1e-5, verbose=False, ftol=1e-6, max_update_norm= None,
        ...
        # obj_val = Helper.geometric_median_objective(median, points, alphas, foolsgold_weight)
        logs = []
        ...
        for i in range(maxiter):
            ...
            weights = torch.tensor([alpha / max(eps, Helper.l2dist(median, p)) for alpha, p, fg_weight in zip(alphas, points, foolsgold_weight)],
                                 dtype=alphas.dtype)
    ...
    # @staticmethod
    # def l2dist_fg(p1, p2):
    #     """L2 distance between p1, p2, with fg weight fg_weight, each of which is a list of nd-arrays"""
    #     squared_sum = 0
    #     for name, data in p1.items():
    #         data = data.to(config.device)
    #         squared_sum += torch.sum(torch.pow(p1[name] - p2[name], 2)) * fg_weight
    #     return math.sqrt(squared_sum)
    ...
    def weighted_average_oracle(points, weights):
        ...
        # foolsgold_weight = torch.tensor(foolsgold_weight)
        # tot_fg_weights = torch.sum(foolsgold_weight)
        ...
        for w, p in zip(weights, points): # 对每一个agent
            for name, data in weighted_updates.items():
                ...
                # temp = temp * (fg_weight/tot_fg_weights).float()
